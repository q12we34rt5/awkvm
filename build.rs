...
fn main() {
    ...
    // (mangled_name, template) pairs to emit.
    ...
    for src in &sources {
        ...
        for (probe_id, mangled) in extract_probe_calls(&ll) {
            let template = templates.get(&probe_id).unwrap_or_else(|| {
                panic!(
            ...
            entries.push((mangled, template.clone()));
        }
    }
...
fn parse_templates(path: &Path) -> HashMap<String, String> {
    ...
    for (lineno, raw) in text.lines().enumerate() {
        ...
        let Some((id, tmpl)) = line.split_once("=>") else {
            panic!(
                "{}:{}: expected `<probe_id> => <template>`, got: {raw}",
                path.display(),
                lineno + 1
            );
        };
        map.insert(id.trim().to_string(), tmpl.trim().to_string());
    }
    map
}
...
fn render_map(entries: &[(String, String)]) -> String {
    let mut s = String::new();
    s.push_str("// Generated by build.rs from probes/. Do not edit by hand.\n");
    s.push_str("pub(super) static PROBE_MAP: &[(&str, &str)] = &[\n");
    for (mangled, template) in entries {
        s.push_str(&format!("    ({:?}, {:?}),\n", mangled, template));
    }
    s.push_str("];\n");
    s
